...
| M3 — Reversible interpreter core | complete | Full M3 command matrix and `bun run validate` passed 2026-07-21 | Optional callback array methods remain intentionally cut per M3 fallback | Preserve runtime invariants while building the table framework | `<pending implementation commit>` |
...
**M3 COMPLETE.** M2 remains live at
`https://board-game-computer.sanocks.workers.dev`. The client now has an Acorn
AST interpreter, transactional forward/inverse patches, exact undo/redo,
bounded execution, canonical state hashes, pending-tail rebase, and a tested
runtime inspector. M4 is the next gate.
