...
| M4 — Table framework and complete deterministic sample | complete | Full M4 command matrix and `bun run validate` passed 2026-07-21 | Pixi bundle emits a non-blocking >500 kB chunk warning | Preserve the deterministic game while adding server-side AI | `<pending implementation commit>` |
...
**M4 COMPLETE.** The deterministic sample now opens at a real Round 3
checkpoint, supports shared human/fallback-AI registered actions, reaches both
explorer victory and vault collapse, renders through Pixi with semantic HTML
controls, and remains exactly undoable. M5 is the next gate.
